...
export function WordPressAPIPage() {
  ...
  const handleGenerateApiKey = async () => {
    ...
    try {
      const response = await apiClient.post(
        `/site-connections/${connection.id}/generate-token`,
      );
      if (response.data.success) {
        setConnection((prev) =>
          prev ? { ...prev, apiToken: response.data.data.apiToken } : null,
        );
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      const response = await apiClient.patch(
        `/site-connections/${connection.id}/toggle`,
      );
      if (response.data.success) {
        setConnection((prev) =>
      ...
      const errorMessage =
        error.response?.data?.error || "Failed to update connection status";
  ...
  const handleUpdateSettings = async () => {
    ...
    try {
      const response = await apiClient.patch(
        `/site-connections/${connection.id}`,
        {
  ...
  const handleTriggerManualScan = async () => {
    ...
    try {
      const response = await apiClient.post(
        `/scanner/trigger/${connection.id}`,
      );
      if (response.data.success) {
        toast.success(
          "Accessibility scan started! Results will be available shortly.",
        );
      }
    } catch (error: any) {
      console.error("Error triggering scan:", error);
      const errorMessage =
        error.response?.data?.error || "Failed to trigger accessibility scan";
      toast.error(errorMessage);
    } finally {
  ...
  const callWordPressScheduleAPI = async (action: string) => {
    ...
    try {
      ...
      const response = await apiClient.post(
        `/wordpress/${connection.id}/schedule`,
        {
          action: action,
        },
        {
  ...
  const handleStartSchedule = async () => {
    ...
    try {
      ...
      setShowPluginErrorModal(true);
      toast.error(
        "Failed to start schedule. Please check if the plugin is installed.",
      );
    } finally {
      setPeriodicScanLoading(false);
    }
  ...
  const handleStopSchedule = async () => {
    ...
    try {
      ...
      setShowPluginErrorModal(true);
      toast.error(
        "Failed to stop schedule. Please check if the plugin is installed.",
      );
    } finally {
      setPeriodicScanLoading(false);
    }
  ...
  const handleRunOnce = async () => {
    ...
    try {
      ...
      setShowPluginErrorModal(true);
      toast.error(
        "Failed to run one-time scan. Please check if the plugin is installed.",
      );
    } finally {
      setPeriodicScanLoading(false);
    }
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-lg text-gray-600 mb-2">{connection.siteName}</p>
            <p className="text-sm text-gray-500">{connection.siteUrl}</p>
          </div>

          <div className="flex items-center space-x-4">
            <div className="flex items-center">
              <div
                className={`w-3 h-3 rounded-full mr-2 ${connection.isActive ? "bg-green-500" : "bg-red-500"}`}
              />
              <span
                className={`text-sm font-medium ${connection.isActive ? "text-green-700" : "text-red-700"}`}
              >
        ...
        <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start">
                      ...
                      <div className="ml-3 text-sm">
                        ...
                        <p className="text-gray-500">
                          Automatically scan for accessibility issues
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start">
                      ...
                      <div className="ml-3 text-sm">
                        ...
                        <p className="text-gray-500">
                          Send notifications when issues are found
                        </p>
                      </div>
                    </div>
          ...
          <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
            ...
            <p className="text-gray-600 mb-6">
              Set up and manage your WordPress plugin integration for
          ...
          >
            ...
            <div className="p-6">
              <p className="text-gray-600 mb-6">
                Before connecting your WordPress site, you'll need to install
                our WordPress plugin. Follow our comprehensive documentation to
                get started.
              </p>
            ...
            <div className="p-6">
              {connection.apiToken ? (
                    ...
                    <div className="flex items-start">
                      ...
                      <div className="ml-3">
                        ...
                        <p className="text-green-700 mb-4">
                          Your WordPress API key is ready to use. Copy this key
                          and enter it in your WordPress plugin settings.
                        </p>
                        ...
                        <div className="bg-white rounded-lg p-4 border border-green-200">
                          <div className="flex items-center justify-between">
                            <code className="text-sm font-mono text-gray-800">
                              {showApiKey
                                ? connection.apiToken
                                : `${connection.apiToken.slice(0, 8)}...${connection.apiToken.slice(-8)}`}
                            </code>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500 mb-6">
                    Generate an API key to connect your WordPress site with our
          ...
          {connection.apiToken && (
            ...
            >
              ...
              <div className="p-6">
                <p className="text-gray-600 mb-6">
                  Monitor your WordPress site for code changes that might affect
                  accessibility compliance. This feature requires the WordPress
                  plugin to be properly installed and configured.
                </p>
                  ...
                  <div className="flex items-center">
                    <div
                      className={`w-3 h-3 rounded-full mr-2 ${
                        scheduleStatus === "schedule-running"
                          ? "bg-green-500"
                          : scheduleStatus === "schedule-not-running"
                            ? "bg-red-500"
                            : "bg-gray-400"
                      }`}
                    />
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <button
                    ...
                    disabled={
                      periodicScanLoading || runStatus === "action-running"
                    }
                  ...
                  >
                    <RefreshCw
                      className={`-ml-1 mr-2 h-5 w-5 ${periodicScanLoading ? "animate-spin" : ""}`}
                    />
                    Refresh Status
                  </button>
                </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="text-sm text-blue-800">
                      <p className="font-medium mb-1">
                        About Code Change Detection
                      </p>
                      <p>
                        This feature monitors your WordPress site for theme and
                        plugin file changes that could impact accessibility.
                        When changes are detected, an automated scan is
                        triggered to check for compliance issues.
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-sm font-medium ${connection.isActive ? "text-green-600" : "text-red-600"}`}
                >
                  {connection.isActive ? "Active" : "Inactive"}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-sm font-medium ${connection.apiToken ? "text-green-600" : "text-red-600"}`}
                >
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3">
                ...
                <p className="text-blue-700 text-sm mb-4">
                  Our WordPress integration provides real-time accessibility
                  monitoring and automated fixes for your site.
                </p>
                <Link
                  to="/help"
            ...
            <div className="mb-6">
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                <p className="text-blue-800 text-sm">
                  <strong>Next Steps:</strong> Please verify that the WordPress
                  plugin is properly installed and configured with your API
                  token.
                </p>
              </div>
            </div>
