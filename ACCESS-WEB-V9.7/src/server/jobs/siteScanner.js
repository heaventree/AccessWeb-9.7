...
const SYNC_INTERVAL_MS =
  parseInt(process.env.SCHEDULE_SYNC_INTERVAL_MS) || 5 * 60 * 1000; // 5 minutes
...
class SiteScannerJobQueue {
  ...
  async initialize() {
    try {
      ...
      console.log(
        "📧 [NOTIFICATION] Notification service ready (with in-app notifications)",
      );

      // Initialize pg-boss with database connection
      this.boss = new PgBoss({
        connectionString: process.env.DATABASE_URL_PG_BOSS,
        schema: "private", // 👈 Force PgBoss to use private schema
        retryLimit: 3,
        retryDelay: 5000,
        expireInHours: 1,
        deleteAfterHours: 24,
      ...
      console.log(
        `⏰ [SITE-SCANNER] Schedule sync interval set to ${SYNC_INTERVAL_MS / 1000} seconds`,
      );
  ...
  async startWorker() {
    try {
      ...
      console.log(
        `👷 [SITE-SCANNER] Worker started with max concurrency: ${MAX_CONCURRENT_JOBS}`,
      );
      console.log(
        `👷 [SITE-SCANNER] Worker listening for jobs on queue: ${JOB_QUEUE_NAME}`,
      );
      ...
      this.boss.on("monitor-states", (states) => {
        if (states && states.queues) {
          const scanQueue = states.queues.find(
            (q) => q.name === JOB_QUEUE_NAME,
          );
          if (scanQueue && scanQueue.count > 0) {
            console.log(
              `📊 [SITE-SCANNER] Queue stats - Active: ${scanQueue.active}, Completed: ${scanQueue.completed}, Failed: ${scanQueue.failed}`,
            );
          }
        }
      });
  ...
  async processScanJob(job) {
    ...
    console.log(
      "🐛 [DEBUG] Processed job object:",
      JSON.stringify(actualJob, null, 2),
    );
    ...
    if (actualJob.data && typeof actualJob.data === "object") {
      ...
      console.error(
        "❌ [SITE-SCANNER] Job data is missing or malformed. Job object:",
        JSON.stringify(actualJob, null, 2),
      );
      throw new Error("Job data is missing or malformed");
    }

    const { connectionId, userId, siteName, siteUrl, platform, frequency } =
      jobData;
...
╚════════════════════════════════════════════════════════════════╝
    ...
    try {
      ...
      const scanResult = await this.performAccessibilityScan(
        connectionId,
        siteUrl,
        platform,
        frequency || "schedule",
...
╚════════════════════════════════════════════════════════════════╝
      ...
      console.log(
        `🔍 [DEBUG] Notification check: scanResult=${!!scanResult}, scanResultId=${scanResult?.scanResultId}, notificationService=true`,
      );

      if (scanResult && scanResult.scanResultId) {
        try {
          console.log(
            `📧 [NOTIFICATION] Attempting to send scan completion notification for user ${userId}`,
          );
          ...
          if (siteConnection) {
            console.log(
              `📧 [NOTIFICATION] Found site connection, sending notification for: ${siteConnection.siteName}`,
            );
            ...
            try {
              await notificationService.createScanCompletedNotification(
                userId,
                {
                  ...
                  criticalIssues:
                    scanResult.errorCount > 10
                      ? Math.floor(scanResult.errorCount * 0.3)
                      : 0,
                  scanId: scanResult.scanResultId,
                },
              );
              console.log(
                `✅ [NOTIFICATION] New notification service used for scan completion`,
              );
            } catch (notificationError) {
              console.log(
                `⚠️ [NOTIFICATION] New service failed, falling back to old method`,
              );
            ...
            console.log(
              `❌ [NOTIFICATION] Site connection not found for ID: ${connectionId}`,
            );
          }
        } catch (notificationError) {
          console.error(
            `❌ [NOTIFICATION] Failed to send scan completion notification:`,
            notificationError,
          );
        ...
        console.log(
          `⚠️ [NOTIFICATION] Skipping notification - missing requirements`,
        );
      }
    } catch (error) {
      console.error(`
...
╚════════════════════════════════════════════════════════════════╝
  ...
  async performAccessibilityScan(
    ...
    try {
      console.log(
        `🔍 [SCAN-${connectionId}] Starting WCAG accessibility scan for: ${siteUrl} (Reason: ${scanReason})`,
      );

      // Perform real accessibility scan using pa11y
      const scanResult = await accessibilityScanner.scanUrl(
        siteUrl,
        connectionId,
        scanReason,
      );
...
╚════════════════════════════════════════════════════╝
      ...
      console.error(
        `❌ [SCAN-${connectionId}] Accessibility scan failed:`,
        error.message,
      );
  ...
  async syncSchedules() {
    try {
      ...
      console.log(
        `📋 [SITE-SCANNER] Found ${activeConnections.length} active connections with auto-scan enabled`,
      );
      ...
      for (const connection of activeConnections) {
        const cronExpression =
          frequencyToCron[connection.scanFrequency] || "0 9 * * 1"; // Default to weekly
        ...
        if (connection.scanFrequency === "testing") {
          console.log(
            `⚡ [SITE-SCANNER] Testing frequency detected for ${connection.siteName} - using 15-second interval trigger`,
          );
        ...
        try {
          ...
          console.log(
            `✅ [SITE-SCANNER] Scheduled job for ${connection.siteName} (${connection.scanFrequency}) - User: ${connection.user.email}`,
          );
        } catch (error) {
          console.error(
            `❌ [SITE-SCANNER] Failed to schedule job for connection ${connection.id}:`,
            error,
          );
        }
      }
      ...
      for (const { connectionId, jobInfo } of jobsToRemove) {
        try {
          ...
          console.log(
            `🗑️  [SITE-SCANNER] Removed job for deleted connection: ${connectionId}`,
          );
        } catch (error) {
          console.error(
            `❌ [SITE-SCANNER] Failed to remove job for connection ${connectionId}:`,
            error,
          );
        }
      }
      ...
      console.log(
        `✅ [SITE-SCANNER] Schedule sync completed - Active jobs: ${this.activeJobs.size}`,
      );
    } catch (error) {
    ...
    try {
      ...
      for (const connection of activeConnections) {
        ...
        try {
          ...
          console.log(
            `🔍 [SITE-SCANNER] Executing direct scan for ${connection.siteName}`,
          );
          ...
          console.log(
            `✅ [SITE-SCANNER] Direct scan completed for ${connection.siteName}`,
          );
          ...
          if (scanResult && scanResult.scanResultId) {
            try {
              ...
              console.log(
                `📧 [NOTIFICATION] Testing scan notification sent for site: ${connection.siteName}`,
              );
            } catch (notificationError) {
              console.error(
                `❌ [NOTIFICATION] Failed to send testing scan notification:`,
                notificationError,
              );
            }
          }
          ...
          console.error(
            `❌ [SITE-SCANNER] Failed to execute direct scan for connection ${connection.id}:`,
            error,
          );
        }
      }
  ...
  async triggerManualScan(jobData) {
    try {
      ...
      console.log(
        `🚀 [SITE-SCANNER] Manual scan job ${jobId} queued for ${jobData.siteName}`,
      );
      return jobId;
    } catch (error) {
  ...
  async sendScanCompletionNotification(userId, scanResult, siteConnection) {
    try {
      ...
      if (!user) {
        console.log(
          `⚠️ [NOTIFICATION] User ${userId} not found, skipping notification`,
        );
        return false;
      }
      ...
      console.log(
        `📧 [NOTIFICATION] User preferences - email: ${emailNotificationsEnabled}, browser: ${browserNotificationsEnabled}, scanCompletion: ${scanCompletionEnabled}`,
      );

      if (!scanCompletionEnabled) {
        console.log(
          `🔕 [NOTIFICATION] User ${userId} has disabled scan completion notifications, skipping`,
        );
        return false;
      }
      ...
      if (browserNotificationsEnabled) {
        inAppCreated = await this.createInAppNotification(
          userId,
          scanResult,
          siteConnection,
        );
      ...
      if (success) {
        console.log(
          `✅ [NOTIFICATION] Scan completion notification sent successfully (email: ${emailSent}, in-app: ${inAppCreated})`,
        );
        return true;
      } else {
        console.log(
      ...
      console.error(
        "❌ [NOTIFICATION] Error in sendScanCompletionNotification:",
        error,
      );
      return false;
    }
  ...
  async createInAppNotification(userId, scanResult, siteConnection) {
    try {
      const { errorCount, warningCount, noticeCount, score, scanStatus } =
        scanResult;
      ...
      console.log(
        `📱 [NOTIFICATION] In-app notification created with ID: ${notification.id}`,
      );
      return true;
    } catch (error) {
      console.error(
...
To manage your notification preferences, visit your account settings.
      ...
      console.log(
        `✅ [NOTIFICATION] Email sent successfully via Gmail: ${info.messageId}`,
      );
      return true;
    } catch (error) {
