...
final class TabUpdates extends Tab {
	...
	public function add_settings() {
		...
		add_settings_field( 'update_server', esc_html__( 'Server location', 'readabler' ), [ $this, 'update_server' ], $group, $section );
	...
	public function update_server(): void {
		...
		UI::get_instance()->render_select(
			...
			$options['update_server'] ?? 'main',
			esc_html__( 'Location', 'readabler' ),
			esc_html__( 'Switch if you are unable to activate the plugin with your purchase code.', 'readabler' ),
			[
				"name" => 'mdp_readabler_' . self::TAB_SLUG . '_settings' . "[update_server]",
			]
		);
