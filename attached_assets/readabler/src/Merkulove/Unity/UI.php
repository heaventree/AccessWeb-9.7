...
final class UI {
	...
	public function render_select( array $options, string $selected = '', string $label = '', string $helper_text = '', array $attributes = [] ) {
        ...
        <div class="<?php echo esc_attr( $class ); ?>">
            ...
            <div class="mdc-select__anchor mdc-select-width">
                ...
                <div id="<?php echo esc_attr( $id ) ?>-text" class="mdc-select__selected-text"
                     aria-labelledby="outlined-select-label"><?php echo esc_html( $options[ $selected ] ); ?></div>
                <div class="mdc-notched-outline">
                    <div class="mdc-notched-outline__leading"></div>
                    <div class="mdc-notched-outline__notch">
						<?php if ( $label ) : ?>
                            <span id="<?php echo esc_attr( $id ) ?>-label"
                                  class="mdc-floating-label"><?php echo esc_html( $label ); ?></span>
						<?php endif; ?>
                    </div>
            ...
            <div class="mdc-select__menu mdc-menu mdc-menu-surface mdc-select-width" role="listbox">
                <ul class="mdc-list">
					<?php foreach ( $options as $key => $value ) : ?>
                        ...
                        <li class="mdc-list-item <?php echo esc_attr( $selected_class ); ?>"
                            data-value="<?php echo esc_attr( $key ) ?>"
                            role="option"><?php echo wp_kses_post( $value ) ?></li>
					<?php endforeach; ?>
                </ul>
            </div>
        </div>
	...
	public function render_colorpicker( $value, string $label = '', string $helper_text = '', array $attributes = [] ) {
        ...
        <div class="mdc-text-field mdc-input-width mdc-colorpicker mdc-text-field--outlined">
            ...
            <div class="mdc-notched-outline">
				...
				<?php if ( $label ) : ?>
                    <div class="mdc-notched-outline__notch">
                        <label <?php echo ( $id ) ? 'for="' . esc_attr( $id ) . '"' : ''; ?>
                                class="mdc-floating-label mdc-floating-label--float-above"><?php echo wp_kses_post( $label ); ?></label>
                    </div>
				<?php endif; ?>
	...
	public function render_snackbar( $message, string $design = '', int $timeout = 5000, bool $closeable = true, array $buttons = [], string $class_name = '' ) {
		?>
        <div class="mdc-snackbar <?php echo ( $design === '' ) ? 'mdc-info' : 'mdc-' . esc_attr( $design ); ?> <?php echo esc_attr( $class_name ); ?>"
                ...
                <div class="mdc-snackbar__actions">
					<?php foreach ( $buttons as $btn ) : ?>
                        <button class="mdc-button mdc-snackbar__action" type="button"
                                onclick="window.open( '<?php echo esc_attr( $btn['link'] ); ?>', '_blank' )"
                                title="<?php echo esc_attr( $btn['caption'] ); ?>"><?php echo esc_html( $btn['caption'] ); ?></button>
					<?php endforeach; ?>
					<?php if ( $closeable ) : ?>
                        <button class="mdc-icon-button mdc-snackbar__dismiss material-icons"
                                title="<?php esc_html_e( 'Dismiss', 'readabler' ); ?>" type="button">close
                        </button>
					<?php endif; ?>
                </div>
            </div>
	...
	public function render_button( string $label = '', string $helper_text = '', $icon = false, array $attributes = [] ) {
        ...
        >
			...
			<?php if ( $icon ) : ?>
                <i class="material-icons mdc-button__icon" aria-hidden="true"><?php echo esc_html( $icon ); ?></i>
			<?php endif; ?>
                ...
                <div class="mdc-switch__thumb">
                    ...
                    <input
						...
						<?php echo ( $name ) ? 'name="' . esc_attr( $name ) . '"' : ''; ?>
                            ...
                            role="switch" <?php echo ( $value === 'on' ) ? 'checked' : ''; ?>>
                </div>
            </div>
        </div>
        <label
			<?php echo isset( $attributes['id'] ) ? 'for="' . esc_attr( $attributes['id'] ) . '"' : '' ?>class="mdc-switch-label"><?php echo esc_html( $label ) ?></label>
	...
	public function render_textarea( $value, string $label = '', string $helper_text = '', array $attributes = [] ) {
		...
		?>
        <div class="mdc-text-field mdc-text-field--textarea mdc-ui-wrap-<?php echo ( $id ) ? esc_attr( $id ) : ''?>">
            <!--suppress HtmlFormInputWithoutLabel -->
            <textarea
                <?php echo ( $id ) ? 'id="' . esc_attr( $id ) . '"' : ' '; ?>
				<?php echo ( $name ) ? 'name="' . esc_attr( $name ) . '"' : ' '; ?>
        ...
        <div <?php echo ( $class ) ? 'class="' . esc_attr( $class ) . '"' : ' '; ?>>
            ...
            <div class="mdc-icon-field-img-box <?php if ( $src ) : ?>mdc-with-image<?php endif; ?>">
                ...
                <button
                        ...
                        data-libraries="<?php echo esc_attr( str_replace('.json', '', join(',', $meta) ) );?>"
	...
	public function render_media_library( $value, string $helper_text = '', array $attributes = [] ) {
		...
		?>
        <div class="<?php echo esc_attr( $class ); ?>" data-uid="<?php echo esc_attr( $id ); ?>"><?php
	...
	public function render_layouts( array $layouts, string $value, string $label = '', string $helper_text = '', array $attributes = [] ) {
        ...
        <div class="mdp-button-dropdown mdp-layout mdc-select-width" data-mdp-dropdown="{mode:'click'}">
            ...
            <button class="mdp-button mdp-button-image" data-val="<?php echo esc_attr( $value ); ?>">
                <img src="<?php echo esc_attr( Plugin::get_url() . 'images/fields/layout/' . esc_attr( $value ) . '.svg' ); ?>"
                     alt="">
                <i class="mdc-select__dropdown-icon"></i>
            </button>
            ...
            <div class="mdp-dropdown mdp-dropdown-width-2 mdp-dropdown-bottom">
                <ul class="mdp-nav mdp-nav-dropdown">
					...
					foreach ( $layouts as $val => $alt ) {
						?>
                        <li>
                            <a href="#" data-val="<?php echo esc_attr( $val ) ?>" <?php if ( $val === $value ) {
								echo 'class="mdp-active"';
							} ?>>
                                <img src="<?php echo esc_attr( Plugin::get_url() . 'images/fields/layout/' . esc_attr( $val ) . '.svg' ); ?>"
            ...
            <div class="mdc-select__menu mdc-menu mdc-menu-surface mdc-select-width" role="listbox">
                <ul class="mdc-list">
					<?php foreach ( $options as $key => $value ) : ?>
						<?php $selected_class = ( $key === $selected ) ? 'mdc-list-item--selected' : ''; ?>
                        <li class="mdc-list-item <?php echo esc_attr( $selected_class ); ?>"
                            data-value="<?php echo esc_attr( $key ) ?>" role="option">
							<?php $img_file = Plugin::get_path() . 'images/fields/select/' . $key . '.svg'; ?>
							<?php if ( file_exists( $img_file ) ) : ?>
                                <img src="<?php echo esc_url( Plugin::get_url() . "images/fields/select/" . $key . ".svg" ); ?>"
        ...
        <div class="mdp-dnd">
            ...
            <div class="mdc-text-field mdc-input-width mdc-text-field--outlined mdc-hidden">
                ...
                <div class="mdc-notched-outline mdc-notched-outline--upgraded mdc-notched-outline--notched">
                    <div class="mdc-notched-outline__leading"></div>
                    <div class="mdc-notched-outline__notch">
                        <label for="mdp-readabler-settings-dnd-<?php echo esc_attr( $key ); ?>"
                               class="mdc-floating-label mdc-floating-label--float-above"><?php echo esc_html( $label ); ?></label>
                    </div>
                    <div class="mdc-notched-outline__trailing"></div>
                </div>
            </div>
            <div id="mdp-<?php echo esc_attr( $key ); ?>-drop-zone"
                 class="<?php if ( $key_exist ) : ?>mdp-key-uploaded <?php endif; ?>mdp-drop-zone">
				<?php if ( $key_exist ) : ?>
        ...
        <div class="mdp-dnd mdp-file">
			...
			echo wp_sprintf(
				'<div class="mdc-text-field mdc-input-width mdc-text-field--outlined mdc-hidden">
					<input  type="text"
							class="mdc-text-field__input mdp-drop-zone-input"
							name="mdp_readabler_%1$s_settings[%2$s]"
							id="mdp-readabler-settings-dnd-%2$s"
							value="%3$s"
					>
					<div class="mdc-notched-outline mdc-notched-outline--upgraded mdc-notched-outline--notched">
						<div class="mdc-notched-outline__leading"></div>
						...
						<div class="mdc-notched-outline__notch">
							<label for="mdp-readabler-settings-dnd-%2$s" class="mdc-floating-label mdc-floating-label--float-above">%4$s</label>
						</div>
						<div class="mdc-notched-outline__trailing"></div>
					</div>
				</div>',
	...
	public function render_sides( $key, $tab_slug, $field ) {
		...
		foreach ( self::$css_sides as $number_key ) {
			?>
            <div class="mdp-controls-sides-single mdp-controls-sides-<?php echo esc_attr( $number_key ); ?>"><?php
			UI::get_instance()->render_input(
				$values[ $number_key ],
				$field['labels'][ $number_key ] ?? esc_html( $number_key ),
				'',
				[
