...
class simple_html_dom_node
{
	...
	function text()
	{
		...
		if (!is_null($this->nodes)) {
			foreach ($this->nodes as $n) {
				...
				// multiple spans don't run into each other. This is a plaintext
				// after all.
				if ($n->tag === 'span') {
					...
					foreach($attributes as $a) {
						...
						if (is_numeric($att_name)
							...
							&& $att_val === '') {
								...
								// Find index of a current element in parent
								foreach ($node->parent->children as $c) {
									if ($c->tag === $node->tag) ++$count;
									if ($c === $node) break;
								}

								// If this is the correct node, continue with the next
								// attribute
	...
	protected function parse_selector($selector_string): array {
		 ...
		 * with an @ sign NOT captured by the expression. Further study
	...
	function hasAttribute($name): bool {
...
class simple_html_dom
{
	...
	function load(
		...
		$defaultBRText = DEFAULT_BR_TEXT,
		$defaultSpanText = DEFAULT_SPAN_TEXT,
		$options = 0): simple_html_dom {

		// prepare
		$this->prepare($str, $lowercase, $defaultBRText, $defaultSpanText);

		// Per sourceforge http://sourceforge.net/tracker/?func=detail&aid=2949097&group_id=218559&atid=1044037
		// Script tags removal now precedes style tag removal strip out <script> tags
